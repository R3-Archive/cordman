...
function eraseBubble(t, x, y) { 
	...
	var size = "";
	if (t === "s") { 
		size = SUPER_BUBBLES_SIZE + 2;
	} else { 
		size = BUBBLES_SIZE;
	}

	ctx.clearRect(x - size, y - size - 2, (size + 1) * 2, (size + 1) * 2);
}

function isSuperBubble(line, bubble) { 
	if ( (line === 23 || line === 1) && (bubble === 1 || bubble === 26)) { 
		return true;
	}
...
function canAddBubble(line, bubble) { 
	...
	if ( ( ( line >= 1 && line <= 4) || (line >= 9 && line <= 10) || (line >= 20 && line <= 22) || (line >= 26 && line <= 28) ) && (bubble === 13 || bubble === 14)) {
		...
		return false;
	} else if ( line === 23 && (bubble === 13 || bubble === 14)) { 
		return false
	}
